package Perl6::Doc::Grant::Proposal;
...
Perl6::Doc::Grant::Proposal - Improving, coordinating and guide self-organization of Perl 6 documentation, a grant proposal.

=head1 Project title

Curation and development toward the next generation of Perl6 documentation.
...
=item working on all outstanding issues and address all of the issues in 2015, 2016 and 2017,

=item creating a set of rules for existing and new contributions and enforce those rules in the test suite,

=item assigning outstanding issues during the grant and create a system for assigning work to volunteers,

=item creating an entry page with a tutorial for complete beginners with very little or no knowledge of programming, with the intent of providing a good landing page for the language

=item contributing to Perl and general open source conferences with entry-level tutorials and learning material, and

=item writing articles in websites such as The Practical Dev (https://dev.to), HackerNoon or Medium with practical "30 minutes" tutorials to perform usual tasks in Perl6.
...
Juan J. Merelo, L<JJ in GitHub|https://github.com/JJ>, L<JMERELO in CPAN|https://metacpan.org/author/JMERELO>, L<jjmerelo in Medium|https://medium.com/@jjmerelo>, L<JJ in The Practical Dev|https://dev.to/JJ>.

=head1 Benefits to the Perl6 community and Perl community at large
...
the L<perl6/doc|https://github.com/perl6/doc> repository used for
...
=item Addressing 5 doc issues per day on average, minimum 2 issues per day. Reduce the amount of open issues by at least 120.

=item Write guidelines for contributions and write test code for enforcing it.

=item Analyze landing pages and site paths and contribute to work on
preferred landing pages. Produce report to improve usability of the
site. 

=item Contribute 1 entry-level tutorial per week, to be published preferably on non-perl publications (Medium, dev.to, hackernoon).

=item Monitor StackOverflow and answer and/or identify needs to be addressed in documentation. Contribute to work on those pages or create new ones.

=item Hang out at the IRC channel to help newcomers in general and
...
=head2 Project details and schedule

L<Perl6 documentation|https://doc.perl.org> is an impressive piece of
...
self-assigned sometimes 
it is difficult to get a particular thing done. This means that there
are issues open since 2015 (#93 since June 13th 2015), and 
almost 300 of them are still open. Even "low hanging fruit" issues are
hanging low for really a long time, with no systematic effort to pick
them up from the tree. 
...
Perl Workshop) if it's considered
necessary. I can organize locally a documentation hackathon by the end
of the grant or do an online squashathon. 

=head2 Project schedule

=over 4

=item Day 7: identify core of people working on the documentation and create an open channel of communication, for instance, a Telegram channel with the GitHub bot connected. Use it for meta-discussions. Then, daily: work on that channel to add a small layer of organization, assign tasks, discuss issues.

=item Daily: monitor StackOverflow, Perl6 IRC channel, Twitter and other social networks for people needing help with Perl6. Identify main sources of problem.

=item As-it-happens: check out PRs for documentation, engage them, accept them if adequate. Check out new issues, engage and assign them to self.

=item Weekly: produce entry-level tutorials and publication on non-perl sites.

=item Daily: work on outstanding perl6/doc issues, starting with the first. Target: 5 a day, minimum 2 a day.

=item Day 15: produce a report on site use and usability. Day 30: implement improvements. Day 45: analyze improvements and produce report on site visits and interactions. Day 60: implement second round of improvements.

=item Day 15: RFC of guidelines for documentation and code included with it. Day 30: code implementation of those guidelines.

=item Day 45: organize online or offline Perl6 documentation hackathon in Granada (Spain) to address outstanding issues, improve code examples and in general add a layer of sustainability to the documentation effort.

=back


=head3 Report schedule

=over 4

=item The grant report will be integrated with the doc GitHub repo, as a ChangeLog or grant report folder. This ChangeLog will be updated as-it-happens.

=item 4 reports, 2 a month, on activity happening outside the log:
...
=head2 Requested amount for the Project Grant

Whatever amount is the usual for two months, part time, work. 

=head1 SEE ALSO

=over 4

=item L<https://docs.perl.org>, main documentation page and its
L<GitHub repository|https://github.com/perl6/doc>.

=item L<Monthly statistics|https://github.com/perl6/doc/pulse/monthly>
for the GitHub repo, showing 42 active issues during that time.

=item L<My contributions to the repo so
